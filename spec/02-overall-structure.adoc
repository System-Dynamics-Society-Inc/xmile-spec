...
The `+<ai_information>+` tag has two REQUIRED and three OPTIONAL sub-tags:

* Required:AI usage status:  `+<status>+` as described in <<sec-ai-status>>. This tag consists of attributes and has no sub-tags.
...
* Required:AI interaction log:  `+<log>+` w/the log of interaction between the user and the AI agent. This SHOULD be plain text, markup, or markdown, in whichever form is most convenient for the software. The `+log_truncated+` attribute of `+<status>+` indicates whether this log has been truncated. This tag MAY be empty but not absent when there is no log of activity.

* Optional:Software-specific settings:  `+<settings>+` w/settings related to AI usage. No sub-tags or attributes are specified, as the nature of these settings is expected to be software specific.

--start claude fix this--* Optional:Agentic AI interaction log:  `+<agentic_log>+` w/the log of interaction between the user an Agentic AI agent where the prompts, replies, and status are delineated. This tag MAY be empty or absent when there is no log of agentic activity. The `+log_truncated+` attribute of `+<status>+` indicates whether this log, or the regular +<log>+ has been truncated. 

 The agentic log consists of one or more +<message>+ tags which each one having two required attributes +<type>+ and +<content>+ and two optional attributes +<is_thinking>+ and +<agent_name>+. All attributes have plain text as their content except for +<content>+ which can also have markup or markdown. 
--end claude fix this--
...
|`+log_truncated+`
|true/false
|True if the log of interactions with the AI agent has ever been truncated. If the software does not store this log as part of its regular activity, this MUST be set to true.
