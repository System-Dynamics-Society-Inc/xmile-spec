...
When the file includes an agentic log, a second signature is created for it. The message to be signed is formed by collating the `+content+` attribute of every `+<message>+` in `+<agentic_log>+`, in document order, and is signed with the same algorithm and keys described above.
