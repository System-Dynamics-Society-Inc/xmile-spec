...
The `+<ai_information>+` tag has three OPTIONAL sub-tags:

* AI interaction log:  `+<log>+` w/the log of interaction between the user and the AI agent. This SHOULD be plain text, markup, or markdown, in whichever form is most convenient for the software. The `+log_truncated+` attribute of `+<status>+` indicates whether this log has been truncated. This tag MAY be empty or absent when there is no log of activity.
