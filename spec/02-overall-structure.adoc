...
The OPTIONAL `+<ai_information>+` tag records status and settings relating to the use of AI in the creation of a model. Its purpose is to ensure that someone opening a model can reliably determine whether, and where, AI was used in creating it. This does not prevent someone from developing a model with AI and then transcribing it into another model by hand, but it does mean that use of AI within conforming software, including through copy and paste, is tracked.
...
|`+mixed+`
|true/false
|Present in all current sample files and included in the signed message string, but not yet defined in this specification. See the editor's question following this table.
...
|`+keyurl+`
|https URL
|A web address that returns, in response to a GET, the base 64 encoded public key for verifying the signature.
...
[NOTE]
====
*EDITOR'S QUESTION (1.2 draft):* `+mixed+` needs a definition. It appears on
`+<status>+` in every sample file, and `+mixed=false+` is present in the signed
message string in `+<testing>+`, so it is already load-bearing for signature
verification, but the source draft for this section does not describe it. What
does it assert? The natural readings are that the model mixes AI generated and
hand built content, or that it mixes trusted and untrusted content, and those are
not the same claim. In all four sample files `+mixed+` is exactly the inverse of
`+verified+`: the two unverified files carry `+mixed="true"+` and the two
verified ones carry `+mixed="false"+`. If that relationship is intended then the
attribute is redundant and could be dropped; if it is coincidental then the
independent meaning needs stating. Is it also irreversible, like the other status
flags?
====
...
|`+type+`
|see below
|The kind of message this is, which distinguishes a user prompt from an agent reply or a status report.
...
[NOTE]
====
*EDITOR'S QUESTION (1.2 draft):* the permitted values of `+type+` are not yet
specified. The sample files use `+user_text+`, `+agent_text+` and
`+agent_complete+`. Is that the complete set, should it be a closed enumeration
that the schema can validate, or is it deliberately open so that vendors can add
their own message kinds? If it is open, a rule is needed for how a reader treats
...
When the file includes an agentic log, a second signature is created for it. The message to be signed is formed by collating the `+type+` attribute of every `+<message>+` in `+<agentic_log>+`, in document order, and is signed with the same algorithm and keys described above.
...
followed by a newline and then the regular log contents, if any. Software that recognises the agentic log MAY validate the signature against the same key. Software that does not recognise it can ignore the prefix, though it is RECOMMENDED that the prefix be stripped before the log is presented to a user. It need not be reinserted when the model is saved.
...
[NOTE]
====
*EDITOR'S QUESTION (1.2 draft):* the text before the colon is not specified. The
sample files use "This model was created or analyzed by a newer version of the
software using an interactive agent". Is that exact string normative, so that a
reader can recognise the prefix reliably, or is it free text that a reader must
locate by structure alone? If it is free text, the note itself may contain a
colon, and a rule is needed for which colon separates the note from the
signature. Specifying a fixed string, or delimiting on the *last* colon, would
remove the ambiguity.
====

[NOTE]
====
*EDITOR'S QUESTION (1.2 draft):* two points about how the two signatures
interact. First, when `+want_log+` is true the `+<log>+` contents are appended to
the main signed message, and those contents now begin with this prefix, so the
agentic signature is itself signed. Is that intended, and is the prefix included
before or after whitespace removal? Second, only the `+type+` of each message is
collated, so `+content+` is unsigned and any message text can be altered without
invalidating the signature. Is signing the sequence of message kinds the intended
guarantee, or should `+content+` be covered too?
====
